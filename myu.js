...
function create_data_array(guil_id,guild_name){
var c_y = String(time.getFullYear());var c_m = String(time.getMonth() + 1);var c_d = String(time.getDate());var c_h = String(time.getHours());	
if(!data_montly[guild_id]){ data_montly[key_arr] = { name: guild_name, data: [] }; }
...
function put_data_array(content){
...
myu.on('message', message => {
		...
		create_data_array(message.guild.id,message.guild.name);	
		console.log(data_montly);	
		}
